...
function remove() {
  ...
  if (y === 6) {
    create('Select a house', arr.houses, 'houseSelector', 'hs', true);
  }
}

export function create(title, arrI, contID, identifier, button) {
  ...
  arrI.forEach((h) => {
    i++;
    document.getElementById(contID).innerHTML += `<img class="houses" id="${
      (identifier, i)
