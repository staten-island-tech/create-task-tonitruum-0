...
function remove() {
  ...
  if (y === 6) {
    create("Select a house", arr.houses, "houseSelector", "hs", true, "houses");
  }
}

export function create(title, arrI, contID, identifier, button, classN) {
  ...
  arrI.forEach((h) => {
    i++;
    document.getElementById(contID).innerHTML += `<img class=${classN} id="${
      (identifier, i)
